...
#include "ui_prfsamplingdialog.h"
...
PrfSamplingDialog::PrfSamplingDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PrfSamplingDialog)
{
    ui->setupUi(this);
...
PrfSamplingDialog::~PrfSamplingDialog()
{
    delete ui;
}

PrfSamplingJob* PrfSamplingDialog::createJob() {
    PrfSamplingJob *job = new PrfSamplingJob();
    ...
    job->setGammaStdev(ui->gammaStep_lineEdit->text().toFloat());
    job->setTauStdev(ui->tauStep_lineEdit->text().toFloat());
    job->setGrubbStdev(ui->grubbStep_lineEdit->text().toFloat());
    job->setRhoStdev(ui->rhoStep_lineEdit->text().toFloat());
...
void PrfSamplingDialog::onDataFileBrowse() {
...
void PrfSamplingDialog::onDataVarSpecBrowse() {
...
void PrfSamplingDialog::onStage2FileBrowse() {
...
void PrfSamplingDialog::pRF_model_changed(int idx) {
    if (idx == 0) { // CSS-pRF
...
void PrfSamplingDialog::verifyDataVariable() {
...
void PrfSamplingDialog::verifyStage2File() {
...
int PrfSamplingDialog::verifyRangeVar(mat_t *mat, const char *varname) {
