...
#include "ui_prfsamplingdialog_canonical.h"
...
PrfSamplingDialogCanonical::PrfSamplingDialogCanonical(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PrfSamplingDialogCanonical)
{
    ui->setupUi(this);
...
PrfSamplingDialogCanonical::~PrfSamplingDialogCanonical()
{
    delete ui;
}

PrfSamplingJobCanonical* PrfSamplingDialogCanonical::createJob() {
    PrfSamplingJobCanonical *job = new PrfSamplingJobCanonical();
...
void PrfSamplingDialogCanonical::onDataFileBrowse() {
...
void PrfSamplingDialogCanonical::onDataVarSpecBrowse() {
...
void PrfSamplingDialogCanonical::onStage2FileBrowse() {
...
void PrfSamplingDialogCanonical::pRF_model_changed(int idx) {
    if (idx == 0) { // CSS-pRF
...
void PrfSamplingDialogCanonical::verifyDataVariable() {
    ...
    matvar_t *mat_top_hrf = 0;
    matvar_t *mat_hrf = QMatFileModel::resolveVariableSpec(mat.data(), "hrf", &mat_top_hrf);
    QSharedPointer<matvar_t> _mat_top_hrf(mat_top_hrf, var_deleter);
    Q_UNUSED(_mat_top_hrf);

    if (mat_hrf == NULL) {
        QMessageBox::warning(this, "Warning",
            QString("Canonical HRF variable not found, the job will fail!")
            .arg(ui->dataVarSpec_lineEdit->text()));
        return;
    }
}

void PrfSamplingDialogCanonical::verifyStage2File() {
...
int PrfSamplingDialogCanonical::verifyRangeVar(mat_t *mat, const char *varname) {
